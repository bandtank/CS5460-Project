...
def dijkstraNavigation():
    ...
    print("GOING", listOfPoints[newPointIndex])
    print("GOING", listOfPoints)
    ...
    while searching:
        for i in range(len(lineIntersctions)):
            ...
            if(float(curPoint[0]) == float(checkPoint[0]) and float(curPoint[1]) == float(checkPoint[1])):
    ...
    while searching:
        for i in range(len(lineIntersctions)):
            ...
            print(checkPoint)
            print(curPoint)
            print(curPoint[0] - checkPoint[0])
            print(curPoint[1] - checkPoint[1])
            if abs(curPoint[0] - checkPoint[0]) < .01 and abs(curPoint[1] - checkPoint[1]) < float(.01):
                print("IN")
                lowestDist = -1
                lowestDest = []
                for j in range(len(totalPoints)):
                    if(float(checkPoint[2]) == float(totalPoints[j][0]) and float(checkPoint[3]) == float(totalPoints[j][1])):
                ...
                for j in range(len(lineIntersctions[i])):
                    curLink = lineIntersctions[i][j]
                    for k in range(len(totalPoints)):
                        if(float(curLink[2]) == float(totalPoints[k][0]) and float(curLink[3]) == float(totalPoints[k][1]) and float(totalPoints[k][2]) < lowestDist):
...
while sim.getSimulationState()!=0:
    ...
    if needNewPoint:
        ...
        if sim.getSimulationState()!=0:
            ...
            for i in range(len(obstacles)):
                ...
                p2 = (obstacles[i][1][0], obstacles[i][1][1])  # (x2, y2)
                # If the edge between the points goes through a wall, it is forced back to the correct side of the wall
                if line_intersection(p1,p2, [objectAbsolutePosition[0], objectAbsolutePosition[1]], listOfPoints[newPointIndex]) == True and needDij == False:
                    print("DIJ")
                    needDij = True
            ...
            if needDij == False:
                ...
                rotation = math.atan2(pointsToTraverse[0][1] - objectAbsolutePosition[1], pointsToTraverse[0][0] - objectAbsolutePosition[0]) - 1.5708
                sim.setObjectOrientation(robot, [0, 0, rotation])
                sim.setObjectOrientation(target, [0, 0, rotation])
            
                if sim.getSimulationState()!=0:
                    time.sleep(1)
                    slightDifX = (pointsToTraverse[0][0] - targetAbsolutePosition[0]) * .25
                    slightDifY = (pointsToTraverse[0][1] - targetAbsolutePosition[1]) * .25
                    while slightDifX > .2 or slightDifX < -.2:
                        slightDifX = slightDifX*.5
                        slightDifY = slightDifY*.5
    ...
    if not needNewPoint and not needDij:
    ...
    if not needNewPoint and needDij:
        if abs(objectAbsolutePosition[0] - checkX) < .5 and abs(objectAbsolutePosition[1] -checkY) < .5:
                print("UP")
                time.sleep(.5)
                if sim.getSimulationState()!=0:
